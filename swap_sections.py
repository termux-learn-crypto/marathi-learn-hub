...
# New sections to add before practiceQuestions
new_sections = """
...
# Escape the pattern for literal string matching (escape [ and ])
escaped_pattern = "    practiceQuestions: ["

# Find and replace
match = re.search(re.escape(escaped_pattern), content)
if match:
    start_pos = match.start()
    print(f"Found at position {start_pos}")
    
    # Replace the first occurrence
    new_content = content.replace(escaped_pattern, new_sections + escaped_pattern, 1)
