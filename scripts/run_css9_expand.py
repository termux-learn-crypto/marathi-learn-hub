...
SLUGS = ["css-aspect-ratio", "css-clamp-fluid", "css-scroll-snap",
         "css-accent-color", "css-color-mix", "css-text-wrap",
         "css-popover-tips", "css-anchor-position", "css-writing-modes",
         "css-modal-backdrop", "css-focus-visible", "css-marathi-capstone9"]
...
def main(frm, to):
    for r in range(frm, to + 1):
        for slug in SLUGS:
            mod = os.path.join(TMP, "adds_css9_%s_r%d.py" % (short(slug), r))
            code = subprocess.Popen(
                [sys.executable, "-c",
                 "import sys; sys.path.insert(0, 'scripts'); import css9_ban; sys.stdout.write(css9_ban.emit_module(%r, %d))" % (slug, r)],
