...
SLUGS = ["css-nav-systems", "css-breadcrumbs", "css-tabs-aria",
         "css-forms-pro", "css-switch-toggle", "css-skeleton-load",
         "css-responsive-table", "css-notifications", "css-print-styles",
         "css-grid-patterns", "css-dark-mode", "css-marathi-capstone8"]
...
def main(frm, to):
    for r in range(frm, to + 1):
        for slug in SLUGS:
            mod = os.path.join(TMP, "adds_css8_%s_r%d.py" % (short(slug), r))
            code = subprocess.Popen(
                [sys.executable, "-c",
                 "import sys; sys.path.insert(0, 'scripts'); import css8_ban; sys.stdout.write(css8_ban.emit_module(%r, %d))" % (slug, r)],
