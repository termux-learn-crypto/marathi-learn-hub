...
# Lessons that need expansion and their practiceQuestions line numbers (1-indexed)
# and the content to insert BEFORE that line
lessons = [
    # (lesson_name, line_idx_0index, new_sections_to_insert)
    # python-threading at line 2312 (0-indexed: 2311)
    ("threading", 2311, """
    {
    ...
    # python-async at line 2436 (0-indexed: 2435)
    ("async", 2435, """
    {
      ...
      content: "chaliye practical example se shuru karte hain. aapko async module import karna hota hai, phir alag alag kaam ke liye alag banayiye. basic syntax: import asyncio. phir async def banavayi. start karne ke liye asyncio.run(main) istemal karein. chaliye ek simple example dekhte hain:\n\ncode: import asyncio\\nasync def mere_kaam():\\n    print(\"Kaam poora hua!\")\\nasyncio.run(mere_kaam()),\noutput: Kaam poora hua!,\n    },
    {
      ...
      content: "async function lwa call karta hai thate await karein. asyncio.run() main coroutine chalta hai. await mdhle program thabbkrto aur I/O che vatt pahoto.\n\nexample: async def compute(): return 40 + 2\\nasync def main(): print(await compute())\\nasyncio.run(main()),\noutput: 42,\n    },
    {
    ...
    # python-pip-advanced at line 2536 (0-indexed: 2535)
    ("pip-advanced", 2535, """
    {
    ...
    # python-practical-project at line 2652 (0-indexed: 2651)
    ("practical-project", 2651, """
    {
      title: "f-strings saaral tarika",
    ...
    },
    {
      ...
      content: "project planning saaral tarika se karein. pehele requirements ladavo, phai design karavo, phai implement karavo. testing madhe variety use kare. maintainable code likavo— variables, functions, comments sahi vart patavi.",
    },
    {
      title: "het adhik sansadnne — Python moreृत (Official)",
...
# Process each lesson
for name, line_idx, new_sections in lessons:
    if line_idx < len(lines):
        # Get the current practiceQuestions line
        pq_line = lines[line_idx]
        
        # Insert new_sections BEFORE the practiceQuestions line
        # The new_sections string starts with a newline, so it will be on a new line
        # The practiceQuestions line follows after
        
        # Insert: new_sections line + original practiceQuestions line
        lines.insert(line_idx, new_sections + pq_line)
        
        print(f"Added sections to {name} at line {line_idx + 1} (was {line_idx})")
    else:
